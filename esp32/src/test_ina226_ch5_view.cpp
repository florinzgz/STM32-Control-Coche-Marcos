// =============================================================================
// Host test for esp32/src/ina226_ch5_view.h (0x318 DIAG_INA_CH5).
// Cross-verifies the ESP32 decoder against the real STM32 classifier/packer.
// =============================================================================
...
static void test_idle_zero_is_ok() {
    std::printf("Scenario: powered but idle zero current stays OK\n");
    Ina226ChannelDiag d = base_snapshot();
    ...
    Ina226Ch5_PackFrame(&d, bytes);
    v5::Ina226Ch5View v;
    CHECK(v5::decode(bytes, 8, v), "decode idle");
    CHECK(v.reason == can::INA_CH5_OK, "idle zero is OK");
    CHECK(v.currentMilliAmps == 0, "idle current zero");
}
...
static void test_cross_decode_reversed() {
    ...
    CHECK(v.shuntMicroVolts == -4000, "shunt uV negative");
    CHECK(v.currentMilliAmps == -2667, "current mA negative");
...
static void test_cross_decode_missing_vs_nd() {
    ...
    CHECK(std::strcmp(v5::notAvailableText(), "CH5 n/d (SIN CONTRATO CAN)") == 0,
          "n/d distinct text");
    CHECK(std::strcmp(v5::statusText(v.reason), v5::notAvailableText()) != 0,
          "MISSING != n/d");
}
...
static void test_present_no_shunt() {
    std::printf("Scenario: present but no shunt drop under demand\n");
    Ina226ChannelDiag d = base_snapshot();
    d.raw_shunt         = 5;
    d.shunt_uv          = 12;
