/**
  ...
  * INA226 hardware CAL register write anywhere in sensor_manager.c.  Saving
  * this store therefore means: sensor_manager.c reads the new per-channel
  * divisor straight from ShuntStore_GetEffectiveMohm() on every
  * Current_ReadAll()/Sensor_UpdateChannel5Diag() cycle, which is exactly the
  * codebase's existing calibration model and takes effect on the very next
  * cycle after Save() (or after Stage(), for host-test observability).
  *
  * Safety invariants:
  *   - NOT editable during an active service-diag test (actuators must be
  *     stopped first) per Block C rules — changing the current divisor
  *     while a wheel is spinning would corrupt the wheel-test measurement
...
/** Stage a candidate set in RAM only.  NOT applied to Current_ReadAll()
 *  until ShuntStore_Save() -- C3 requires actuators stopped (Block C rule),
 *  so there is no "live during test" consumer for the staged-but-unsaved
 *  value. */
