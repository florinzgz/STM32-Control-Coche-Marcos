...
/* Safety_GetState() is called by ShuntStore_Save(); provide a stub whose
 * return value the test can flip, to exercise the STANDBY-only flash-write
 * gate (rejects outside STANDBY, accepts inside STANDBY). */
