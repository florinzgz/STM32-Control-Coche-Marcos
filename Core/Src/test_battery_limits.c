...
int main(void)
{
    ...
    /* Coherent trip ordering: Cutoff < Limit <= Warning. */
    ...
    /* Recovery is an independent SAFE hysteresis threshold.  It may sit below
     * Limit, but can never be at/below Cutoff. */
    b = defaults(); b.recovery_cv = b.cutoff_cv + 1U; ASSERT_TRUE(BatteryLimits_ValidateValues(&b));
