...
**Revision:** 1.13
**Status:** ACTIVE — Added per-wheel speed-sensor fault-reason diagnostic (0x313, additive)
**Date:** 2026-07-05
...
| 0x313 | DIAG_WHEEL_SENSOR | 8 | 1000 ms | Per-wheel speed-sensor fault-reason diag: byte0-3=reason FL/FR/RL/RR (WheelDiag_t code 0-8), byte4=reason STEER/CENTER (reserved, 0=OK), byte5=gpio_level_mask (bit0 FL..bit3 RR, bit4 STEER), byte6=active_fault_mask (bit0 FL..bit3 RR, bit4 STEER), byte7=flags/seq (bit0 powertrain_engaged, bit1 manual_movement, bit2 wheel_fault_debouncing, bit3 wheel_fault_latched, bits4-7 sequence). Reason codes: 0=OK 1=NO_PULSE 2=STUCK_HIGH 3=STUCK_LOW 4=MISMATCH 5=IMPOSSIBLE_RATE 6=MANUAL_MOVEMENT 7=DISABLED_STATE 8=UNKNOWN. ESP32 accepts DLC≥7 (byte7 read only at DLC 8); DLC<7 dropped. Diagnostic-only — no control/safety path consumes it; wheel-fault safety logic unchanged. | `can_handler.c`, `safety_system.c`, `main.c` |
