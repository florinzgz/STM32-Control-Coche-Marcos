...
> ⚠ **ACTUALIZACIÓN CAN rev 1.4 (2026-04-23):** La secuencia de relés pasa de 3 fases (MAIN→TRAC→DIR) a **2 fases (TRAC→DIR)**. El hardware real no tiene contactor MAIN/Power-Hold. Bitmap de `relay_status` en heartbeat 0x001 byte 5: bit 0 = TRAC, bit 1 = DIR, bit 7 = SEQ_COMPLETE. `MODULE_RELAY_MAIN` renombrado a `MODULE_RELAY_TRAC` (ID 3 preservado). Ver `CAN_CONTRACT_FINAL.md`.
