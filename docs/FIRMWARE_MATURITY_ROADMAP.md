...
| 1.4 | **Sensor plausibility checks** — Cross-validate temperature readings (reject if any sensor deviates >30°C from median). Validate wheel speed consistency (if 3 wheels agree and 1 disagrees, flag the outlier). ✅ **PARTIAL (warning subset)** — wheel-speed median-outlier check added in `Safety_CheckSensors`: when all four wheels report > 5 km/h and one deviates > 50 % from the median of the other three, the offending `MODULE_WHEEL_SPEED_*` is tagged `MODULE_FAULT_WARNING`. Diagnostic only — no `fault_count` increment, no DEGRADED entry. Temperature cross-validation still pending. | Medium |
