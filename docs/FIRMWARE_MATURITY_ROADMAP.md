...
| **Emergency stop via obstacle** | obstacle_safety.cpp: triggerEmergencyStop() with immediate motor cut + relay open | **Implemented** *(2026-04-21: 50 cm policy)*: Distance < 500 mm → obstacle_scale = 0.0, SAFE state (motors stopped, steering centered). CAN timeout and sensor failure also trigger SAFE state. Recovery with hysteresis (>750 mm for >1 s). | Gap closed for STM32 backstop layer. |
