...
| **Wheel Speed Processing** | FULL | 4× LJ12A3 inductive sensors via EXTI interrupts, 1 ms debounce, speed = (pulses/CPR) × circumference × 3.6. 6 pulses/rev, 1.110 m circumference (185 mm/pulse). |
...
| **Obstacle Detection** | STM32: BACKSTOP / ESP32: FULL *(Updated 2026-04-21: 50 cm policy)* | STM32 receives distance via CAN (0x208) and applies 5-tier backstop limiter (<500/500/1000/1500/2000/4000 mm → scale 0.0/0.3/0.7/0.85/0.95/1.0). CAN timeout (500ms) and stale-data detection trigger SAFE state. Full 5-zone detection with UART sensor parsing, child reaction, ACC remains ESP32-side only. See `docs/OBSTACLE_SYSTEM_ARCHITECTURE.md`. |
| **Obstacle Safety / Avoidance** | STM32: BACKSTOP / ESP32: FULL *(Updated 2026-02-13)* | STM32 safety backstop: obstacle_scale applied in torque pipeline; emergency distance sets forward_blocked (no SAFE state); CAN timeout or sensor fault sets scale=0.3. ESP32-side only: 5-zone speedReductionFactor, child reaction detection, parking assist, adaptive cruise coordination, audio alerts. |
...
| **Emergency stop via obstacle** | obstacle_safety.cpp: triggerEmergencyStop() with immediate motor cut + relay open | **Implemented** *(2026-04-21: 50 cm policy)*: Distance < 500 mm (confirmed ≥200 ms) → obstacle_scale = 0.0, forward blocked, `SAFETY_ERROR_OBSTACLE` set — **no SAFE state transition**. CAN timeout (active obstacle) → scale=0.3, `OBS_STATE_SENSOR_FAULT`. Sensor failure → scale=0.3. Recovery with hysteresis (>750 mm for >1 s). | Gap closed for STM32 backstop layer. |
...
| 1.4 | **Sensor plausibility checks** — ✅ COMPLETE. **Wheel speed**: when all four wheels report > 5 km/h and one deviates > 50 % from the median of the other three, the offending `MODULE_WHEEL_SPEED_*` is tagged `MODULE_FAULT_WARNING` (in `Safety_CheckSensors`). **Temperature**: when ≥ 3 enabled DS18B20s have valid readings, each sensor is compared against the median of the others; deviation > 30 °C tags the offending `MODULE_TEMP_SENSOR_i` with `MODULE_FAULT_WARNING`. Both checks are diagnostic-only — no `fault_count` increment, no DEGRADED entry; absolute thresholds in `Safety_CheckTemperature()` remain authoritative. | Medium |
| 1.5 | **DS18B20 fault tolerance** — ✅ COMPLETE. Per-sensor `temp_read_invalid[]` flag set on CRC failure or out-of-range read; `Temperature_ReadAll()` preserves the last good value instead of stamping 0.0 and tags `MODULE_TEMP_SENSOR_i` with `MODULE_FAULT_ERROR`. New `Temperature_IsValid(idx)` accessor; `Safety_CheckTemperature` and `Safety_CheckSensors` skip invalid sensors. Existing `Safety_SetError(SAFETY_ERROR_SENSOR_FAULT)` retained — safety envelope unchanged. | Low |
...
| 2.3 | **Temperature rate-of-change monitoring** — ✅ COMPLETE. Per-DS18B20 `|dT/dt|` tracker in `Temperature_ReadAll()` raises `MODULE_FAULT_WARNING` when the slope exceeds `TEMP_RATE_WARN_C_PER_S` (5 °C/s). 2 s latch keeps the warning visible across the 1 Hz `SERVICE_FAULTS` (0x301) frame even though `Safety_CheckTemperature()` clears in-range faults at 100 Hz. Pure diagnostic — no state-machine change, no `fault_count` contribution. | Low |
